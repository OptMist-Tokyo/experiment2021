...
## What's this?
- 議事録・進捗まとめ
- ソースコードはこちら:[github](https://github.com/OptMist-Tokyo/experiment2021)
- hackmdで保存したログをコピペしている
...
- オンライン・オフラインハイブリッド
    - 感染対策はしっかりと
- 発表
    - 途中デモ動画を流すときにハウリングでトラブルがあったが、発表は練習通り終了
    - 質疑応答
        - 先生方からいくつかアルゴリズム的な質問を頂いた
        - 学生からはハードウェアに関して質問をいくつか
        - ついでにスマホから噴水の色が制御できるようなものを作ったのでそれのデモもやってみた

## 6/18 第九回
- 本日の進捗
    - デモ動作撮影と発表準備
    - 発表練習
        - 先生方から質問していただき、質疑応答の練習
...
- 休日活動
- モータードライバ(MD)を作る
    - MDって言っても片方向回せばいいだけだから難しくない
    - リレーを用いると簡単に解決するが、拡張性を考えると良くない
        - 例えばPWMを用いた制御は不可能
    - フォトカプラで絶縁 + FETで駆動の形を取る
        - フォトカプラは使ったことのあるPS817を秋月で買ってきた
        - FETは高校の頃から愛用しているTK13A60Dを使用
            - 多分オーバースペック(不用意に高い耐圧はオン抵抗を上げるだけなので良くない)
            - こいつなら放熱板付けときゃ大体耐えるという信頼があるので採用
            - ちなみに筆者は1回400Aを流したことがあり、その際はこのFETが物理的に真っ二つに割れた
        - エンハンスメント型なので適当にゲートをプルダウンすれば使える。便利でいいね！
- 回路図が出来たのではんだ付け
    - ユニバーサル基板に
- 動作確認用にモーターと並列な配置にLEDを追加
    - これ配線のデバッグにも役立った(LED光ってないからモーターで短絡が生じてるねとか)
...
- 決めること
    - 次回の日程
        - 6/11は各自ラズベリーパイを買いに行く？
        - -> 6/18
    - 発表はオンライン？オフライン？
        - デモについて
            - デモ動画のほうがやりやすい
            - セミナーADに水を撒き散らす危険性がある
            - -> セミナーADで実物のデモは出来ない
        - 参加形式
            - 他の班はオンサイトが多い
            - 動画をその場で流しつつ、動かさないにしても実物を見せる
- 本日の進捗
    - ひたすら配線をつなぎました
    - 配線が多すぎて終わりませんでした
...
- 仕様確認
    ...
    - ポンプ
      - 10-6で分割
- 発表について仕様検討
    - オンラインなので、まずカメラを通すのは前提
    - 案: デモ動画を見せる1
        - メリット
            - いいカメラ配置などを検討する十分な時間が取れる
            - 「かっこよく見せる」には編集をしたほうが良い
            - 曲を変える/用途を変えるという演出にも編集が向いてる
            - リアルタイムに見せようとして動かない可能性を排除できる
        - その他
            - この形式だと来週はスライド作成班と動画撮影・編集班に分かれる
- 今日やること
  - 目標: ハード面の完成
  - 以下やること
    - LED配線
        - できれば配線の色とLEDの色の対応を統一出来ると良い
    - ブレッドボードに抵抗を指す
      - 抵抗の種類を分けると赤が弱すぎたので、全部100Ωで統一
          - 緑の線が入ってない方
  - その他のTODO: モーター駆動回路作る
### 今日やったこと
- ハード面の仕上げ
...
- スライド
    - 先人のもの
        - [2019](https://github.com/OptMist-Tokyo/experiment2019/blob/master/%E3%83%A9%E3%82%BA%E3%83%91%E3%82%A4%E5%AE%9F%E9%A8%93.pdf)
        - [2018](https://docs.google.com/presentation/d/1L3Qbfo88707mW1yl8G_C0_assK0MUAWj8IwyNaLSIhU/edit?usp=sharing)
    - つくる
        - [スライドのURL](https://docs.google.com/presentation/d/1kPS3nUIsMiIy71GyeFpBmSgpkFlwxitCwU73nL2a9p0/edit?usp=sharing)
    - 構成
        - Introduction(導入・目的)
            - ラズパイとは？何ができるか？ （高倉）
                - 前のスライドを引用しつつ
...
- 進捗確認
  - ハード班
    - 設計図を書いてきてもらった
  - ソフト班
    - ハード班回路パートと協力してLEDを光らせた
      - 回路の作り方やPWMについて学んだ
    - 音->光変換のプログラムからLEDを光らせることができた
- 今日やること
  - 当面の目標
    - 残り回数が発表練習まで3回
        - 発表練習の前にスライド作成、見せ方の検討が必要
        - 1回くらいかかりそう
    - あと2回で動くものを作りたい
        - ソフトは大体形になったけどハードはまだ全然終わっていない
        - 今日はソフト班にハードを手伝ってほしい :ojigineko-fast:
  - ハード班のTODO
    - 発泡スチロール加工(噴水の筐体作成)
    - 水槽の組み立て(LEDやパイプ)
    - 水槽
...
## 反省
- 初回のアイデア出しで比較的話が発散してしまった
    - 最初に時間配分を考え、状況次第で時間配分をフィードバック制御すると良い
        - フィードフォワードしかしていなかった
    - 役割は最初に振っておくと良い
        - 議長、書記、タイムキーパー
- 班割りが上手く出来るとよい
    - 一回タスクの全体像を洗い出して振ると良い
- マネジメントについて
    - マネジメントは1人振って良いくらいやるべき
        - マネジメントってなに？
        - 漢字4文字の中で最も意味が近いのは"進捗管理"
    - あまり実装に忙しくならず、でも全体的な実装が大まかに見通せる人がやると良い
        - そんな人は存在するのか？
